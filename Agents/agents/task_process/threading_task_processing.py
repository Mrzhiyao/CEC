...
from node_send_llm import send_file_llm
...
from vector_database.search_milvus import search_data
import glob
import os
...
def task_processing(path,path_des,i,oasst1_val):
    print('iiiiii:', i)
    ...
    if len(sub_execution_files) != 0:
        for sub_execution_file in sub_execution_files:
            ...
            ppp_tasks_list = out.split('\n')
            for content in ppp_tasks_list[:-1]:
                if content.split(' ')[1] == '192.168.2.7':
            ...
            if file_task in files:
                ...
                if  file_task != last_file_name:
                    ...
                    if len(sub_execution_files) == 0:
                        ...
                        memory_used_val = float(memory_used_val)
                        
                        if gpu_utilization<=70 and memory_utilization<=70 and memory_used_val<=5600:
                            re_content = search_data(oasst1_val, task)
                            task = 'Reference documents that may be useful:\n' + re_content[:300] + '\n' + task
                            if len(task)>1000:
                                task = task[:1000]
                            tasks = get_tasks_local(task)
                            crew = Crew(
                                agents=[agents_local.Comprehensive(), agents_local.KnowledgeQuery(), agents_local.ParagraphRetrieve(), agents_local.QA(), agents_local.Calculater(), agents_local.Code_generate()],
                                tasks=tasks,
                        ...
                        if sendip != '192.168.2.7':
                            print('需要转发任务' + file_task + ', 任务目标地:' + sendip)
                            if sendip == '192.168.2.26':
                                send_file_llm(_FILE_RESULT + '/' + file_task, '192.168.2.7')   
                            elif sendip == '192.168.2.8':
