...
class ActorCritic(nn.Module):
    ...
    def get_action(self, state, eval=False):
        ...
        vram_allocated = torch.cuda.memory_allocated() / (1024 )
        final_ram = psutil.Process().memory_info().rss / (1024 )
        # rams.append(final_ram - initial_ram)
        # vrams.append(vram_allocated-vram_allocated1)
        print('ram',final_ram - initial_ram)
        print('vrams',vram_allocated-vram_allocated1)
...
class PPO:
    def __init__(self, state_dim_critic, state_dim_actor, action_dim, actor_node, critic_node, lr_actor, lr_critic,
        ...
        self.policy = ActorCritic(state_dim_critic, state_dim_actor, action_dim, actor_node, critic_node, self.device).to(
            self.device)
        # self.policy.load_state_dict(torch.load("model_network_initial.pth"))
        ...
        # self.policy_old.load_state_dict(torch.load("model_network_initial.pth"))
        # print('载入初始化模型')
    ...
    def learn(self, steps, experiences):
        ...
        if steps % 100 == 0 and steps !=0:
            torch.save(model_policy.state_dict(), 'model_network_722.pth')
            print('模型保存成功')
