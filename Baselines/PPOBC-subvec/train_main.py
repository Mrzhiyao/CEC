...
def get_config():
    ...
    parser.add_argument("--run_name", type=str, default="PPOBC", help="Run name, default: PPOBC")
...
def train(config):
    ...
    expert_data = ReplayBuffer(buffer_size=config.buffer_size, batch_size=config.batch_size, device=device)
    ...
    collect_random_main_train_test(dataset=expert_data)

    agent = PPOBC(state_dim_actor, action_dim, actor_node, lr_actor,lr_critic, device, expert_data.sample(1296))
    ...
    with wandb.init(project="add-algorithm", name=config.run_name, config=config):
        ...
        for i in range(0, config.episodes):
            ...
            for t in range(500):
                ...
                if len(buffer)>300:
                    for ttt in range(20):
                        policy_loss_list, bc_loss_list, combined_actor_loss_list, value_loss_list = agent.learn(t, buffer.sample(), learn_nums)
                        policy_loss_tensor = torch.stack(policy_loss_list)
                        bc_loss_tensor = torch.stack(bc_loss_list)
                        combined_loss_tensor = torch.stack(combined_actor_loss_list)
                        value_loss_tensor = torch.stack(value_loss_list)
                        wandb.log({
                            "policy_loss": policy_loss_tensor.mean().item(),
                            "bc_loss": bc_loss_tensor.mean().item(),
                            "combined_loss": combined_loss_tensor.mean().item(),
                            "value_loss": value_loss_tensor.mean().item()
                        })
                    learn_nums = learn_nums + 1
