...
def get_config():
    ...
    parser.add_argument("--run_name", type=str, default="PPO-nothing", help="Run name, default: PPO")
    parser.add_argument("--env", type=str, default="'MyCustomEnv-v0", help="Gym environment name, default: MyCustomEnv-v0")
    parser.add_argument("--episodes", type=int, default=1, help="Number of episodes, default: 200")
...
def sort_by_sixth_from_end(lst):
    def get_sixth_from_end(s):
        return int(s[-5])

    sorted_lst = sorted(lst, key=get_sixth_from_end)
    return sorted_lst
...
def train(config):
    ...
    with wandb.init(project="RL comparsion", name=config.run_name, config=config):
        ...
        path_result1 = _os.path.join(_base, 'node_receive_resource', 'node1')
        path_time1 = _os.path.join(_base, 'node_receive_resource', 'time', 'node1')
        path_subtask1 = _os.path.join(_base, 'subtasks')

        path_result2 = _os.path.join(_base, 'node_receive_resource', 'node2')
        path_time2 = _os.path.join(_base, 'node_receive_resource', 'time', 'node2')
        path_subtask2 = _os.path.join(_base, 'subtasks')

        path_result3 = _os.path.join(_base, 'node_receive_resource', 'node3')
        path_time3 = _os.path.join(_base, 'node_receive_resource', 'time', 'node3')
        path_subtask3 = _os.path.join(_base, 'subtasks')

        path_result4 = _os.path.join(_base, 'node_receive_resource', 'node4')
        path_time4 = _os.path.join(_base, 'node_receive_resource', 'time', 'node4')
        path_subtask4 = _os.path.join(_base, 'subtasks')
                ...
                if (t+1) % 10 == 0 and t != 0:
                    ...
                    for task_file in tasks_list:
                        ...
                        try:
                            ...
                            matching_files_result1 = find_files_containing_name(path_result1, task_file[:-6])
                            matching_files_result2 = find_files_containing_name(path_result2, task_file[:-6])
                            matching_files_result3 = find_files_containing_name(path_result3, task_file[:-6])
                            matching_files_result4 = find_files_containing_name(path_result4, task_file[:-6])

                            results_all = []
                            time_results_all = []

                            if len(matching_files_result1)!= 0:
                                for i in range(len(matching_files_result1)):
                                    results_all.append(path_result1 + '/' + matching_files_result1[i])

                            if len(matching_files_result2)!= 0:
                                for i in range(len(matching_files_result2)):
                                    results_all.append(path_result2 + '/' + matching_files_result2[i])

                            if len(matching_files_result3)!= 0:
                                for i in range(len(matching_files_result3)):
                                    results_all.append(path_result3 + '/' + matching_files_result3[i])

                            if len(matching_files_result4)!= 0:
                                for i in range(len(matching_files_result4)):
                                    results_all.append(path_result4 + '/' + matching_files_result4[i])

                            results_all = sort_by_sixth_from_end(results_all)

                            time_matching_files_result1 = find_files_containing_name(path_time1, task_file[:-6])
                            time_matching_files_result2 = find_files_containing_name(path_time2, task_file[:-6])
                            time_matching_files_result3 = find_files_containing_name(path_time3, task_file[:-6])
                            time_matching_files_result4 = find_files_containing_name(path_time4, task_file[:-6])

                            time_results_all = []

                            if len(time_matching_files_result1)!= 0:
                                for i in range(len(time_matching_files_result1)):
                                    time_results_all.append(path_time1 + '/' + time_matching_files_result1[i])

                            if len(matching_files_result2)!= 0:
                                for i in range(len(time_matching_files_result2)):
                                    time_results_all.append(path_time2 + '/' + time_matching_files_result2[i])

                            if len(matching_files_result3)!= 0:
                                for i in range(len(time_matching_files_result3)):
                                    time_results_all.append(path_time3 + '/' + time_matching_files_result3[i])

                            if len(matching_files_result4)!= 0:
                                for i in range(len(time_matching_files_result4)):
                                    time_results_all.append(path_time4 + '/' + time_matching_files_result4[i])
                                    
                            time_results_all = sort_by_sixth_from_end(time_results_all)

                            for x in range(len(results_all)):
                                with open(results_all[x], 'r') as file:
                                    content = file.read()
                                result_sim = client.search(data=[embedding(content)], collection_name=collection_name, output_fields=["id", "text"], search_params=search_params,limit=1)
                                completion = result_sim[0][0]['distance']
                                with open(time_results_all[x], 'r') as file:
                                    end_time_sub = file.read()
                                st = task_file[-6][0:10] + '.' + task_file[-6][10:]
                                spend_time_sub = float(end_time_sub) - float(st)

                                if float(completion) < 0.7:
                                    rewards_sub.append((-spend_time_sub) * 2 - (0.7-float(completion))*10)
                                else:
                                    rewards_sub.append( (-spend_time_sub) * 2)
                            ...
                            rewards_mem = -250
                            list_rewards.append(rewards_mem)
                            for x in range(len(state_mem[add_number])):
                                rewards_sub.append(-250)
                            pass
                        ...
                        for ad in range(len(state_mem[add_number])):
                            ...
                            if ad == len(state_mem[add_number])-1:
                                if action_mem[add_number][ad] == 3:
                                    print('cloud')
                                    buffer.add(state=result_state_new, action=action_mem[add_number][ad], reward=rewards_sub[ad] - 30, next_state=result_next_new, done=0)
                                else:
                                    buffer.add(state=result_state_new, action=action_mem[add_number][ad], reward=rewards_sub[ad], next_state=result_next_new, done=0)
                            else:
                                buffer.add(state=result_state_new, action=action_mem[add_number][ad], reward=rewards_mem-action_mem[add_number][ad] * 30, next_state=result_next_new, done=0)
