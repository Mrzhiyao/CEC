...
def get_config():
    ...
    parser.add_argument("--run_name", type=str, default="ppo-vectorkeywords", help="Run name, default: ppo-vectorkeywords")
        ...
        for i in range(0, config.episodes):
            ...
            for t in range(500):
                ...
                for m in range(len(state)):
                    ...
                    original_result_state_add = result_state[0:15] + result_state[15:19] + result_state[35:39] + result_state[55:59]
                    # print(original_result_state_add)
                    indices_to_extract = list(range(0, 15)) +  [15, 18, 19, 22, 23, 26]  # 0-14, 15, 19, 23 (共18个)

                    # 提取特定元素组成新列表
                    result_state_add = [original_result_state_add[i] for i in indices_to_extract]

                    # 去掉GPU部分，每个节点的查询结果只保留一个最相似的
                ...
                for ttt in range(10):
                    loss, output_policy_loss, output_value_loss, output_loss = agent.learn(steps, buffer.sample())
                ...
                if (t+1) % 10 == 0 and t != 0:
                    ...
                    for task_file in tasks_list:
                        ...
                        for ad in range(len(state_mem[add_number])):
                            ...
                            original_result_next_new = result_next[0:15] + result_next[15:19] + result_next[35:39] + result_next[55:59]
                            indices_to_extract = list(range(0, 15)) +  [15, 18, 19, 22, 23, 26]  # 0-14, 15, 19, 23 (共18个)
                            result_next_new = [original_result_next_new[i] for i in indices_to_extract]

                            buffer.add(state=result_state_new, action=action_mem[add_number][ad], reward=rewards_mem-action_mem[add_number].count(3) * 30, next_state=original_result_next_new, done=0)
