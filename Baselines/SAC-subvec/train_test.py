...
def train(config):
    ...
    actor_node = 512
    critic_node= 512
    K_epochs = 50         # update policy for K epochs in one sac update
    ...
    with wandb.init(project="algorithm1-subvec", name=config.run_name+'training-sac', config=config):
        for xx in range(200):
            print('step:', xx)
            policy_loss, alpha_loss, bellmann_error1, bellmann_error2, current_alpha = agent.learn(xx, buffer.sample(), gamma=0.99)
            print(policy_loss)
            ...
            torch.save(checkpoint, 'sac_pretain.pth')
