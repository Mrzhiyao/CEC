...
import glob
from utils import collect_random_main_train_test, save_sac
import random
...
def train(config):
    ...
    agent = SAC(state_dim_actor, action_dim, actor_node, lr_actor, device)
        ...
        for i in range(0, config.episodes):
            ...
            for t in range(500):
                ...
                for ttt in range(10):
                    ...
                    policy_loss, alpha_loss, bellmann_error1, bellmann_error2, current_alpha = agent.learn(t, buffer.sample(), gamma=0.99)
                    print(policy_loss)
                ...
                if i % config.save_every == 0:
                    save_sac(config, save_name="SAC_discrete", model=agent.actor_local, wandb=wandb, ep=0)
