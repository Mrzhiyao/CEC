...
def save_DDPG(args, save_name, model, wandb, ep=None):
    # 定义基础保存目录
    save_dir = './trained_models/'
    ...
    # 1. 始终保存到本地目录
    base_file_name = f"{args.run_name}{save_name}"
    
    if ep is not None:
        local_filename = os.path.join(save_dir, f"{base_file_name}{ep}.pth")
    else:
        local_filename = os.path.join(save_dir, f"{base_file_name}.pth")
    
    # 保存模型到本地
    torch.save(model.state_dict(), local_filename)
    
    # 可选：添加额外的日志记录
    print(f"模型已保存至: {local_filename}")
    # wandb.log({"model_save_path": local_filename})
