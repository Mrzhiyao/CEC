...
import glob
from utils import collect_random_main_train_test, save_DDPG
import random
    ...
    with wandb.init(project="add-algorithm", name=config.run_name, config=config):
        ...
        for i in range(0, config.episodes):
            ...
            for t in range(500):
                ...
                for ttt in range(10):
                    critic_loss, actor_loss = agent.learn(t, buffer.sample(), gamma=0.99)
                ...
                if i % config.save_every == 0:
                    save_DDPG(config, save_name="DDPG_discrete", model=agent.actor_local, wandb=wandb, ep=0)
