...
class MyCustomEnv(gym.Env):
    ...
    def step_new(self, sta_task_number, state, task_content_list, state_relation, actions1, actions2, task_time):
        new_task_array = []
        print('state_relation,state_relation', state_relation)
        for i in range(len(state_relation)):
            if 0 not in state_relation[i]:
                continue
            if actions2[i] == 1:
                # 使用列表推导式和enumerate来找到值为1和2的索引位置
                indexes_of_0_and_1 = [index for index, value in enumerate(state_relation[i]) if value in {0, 1}]
                print("索引位置:", indexes_of_0_and_1)
                if len(new_task_array) == 0:
                    new_task_array.append(indexes_of_0_and_1)
                else:
                    # 遍历可能的任务重组列表
                    for t in range(len(new_task_array)):
                        # 查看当前存在的任务重组安排和接下来的动作是否有任务重合
                        # print(set(indexes_of_0_and_1), set(new_task_array[t]), indexes_of_0_and_1, new_task_array[t])
                        print('set',set(indexes_of_0_and_1), set(new_task_array[t]))
                        if set(indexes_of_0_and_1) & set(new_task_array[t]) != 0:
                            # 进一步添加子任务
                            # print('indexes_of_0_and_1', indexes_of_0_and_1)
                            # print('new_task_array', new_task_array[t])
                            for u in indexes_of_0_and_1:
                                if u not in new_task_array[t]:
                                    new_task_array[t].append(u)
                            # print(new_task_array[t])
                        else:
                            # print('all new', new_task_array)
                            new_task_array.append(indexes_of_0_and_1)
                print('midnew_task_array', new_task_array)
        # print('new_task_array', new_task_array)
        new_task_array_sort = []
        for nr in new_task_array:
            new_task_array_sort.append(sorted(nr))
        # print('new_task_array_sort', new_task_array_sort)
        # print('tasklist', self.current_task_list)
        ...
        print('action_carry', action_carry)
        filename = dis_task(sta_task_number, sorted_task_list, action_carry, task_time, state_relation, task_content_list)
        # filename = 'filename'
        # last_number = 0
        # for nr in new_task_array_sort:
        #     last_number = last_number + max(nr)-min(nr)
        # print('task_number', len(sorted_task_list), last_number)
