...
def get_config():
    ...
    parser.add_argument("--run_name", type=str, default="IRL", help="Run name, default: IRL")
...
def train(config):
    ...
    expert_demon = ReplayBuffer(buffer_size=config.buffer_size, batch_size=config.batch_size, device=device)
    ...
    collect_random_main_train_test(dataset=expert_demon)
    expert_states, expert_actions, expert_rewards, expert_next_states, expert_masks = expert_demon.sample(len(expert_demon))

    expert_states_actions = torch.cat((expert_states, expert_actions), dim=1)

    agent = IRL(state_dim_actor, 1, actor_node, lr_actor, device)
    ...
    with wandb.init(project="add-algorithm", name=config.run_name, config=config):
        ...
        for i in range(0, config.episodes):
            ...
            for t in range(500):
                ...
                if len(buffer)>100:
                    """更新网络参数"""
                    discrim_loss, value_loss = agent.update_params(expert_states_actions, buffer.sample(100), t)
                    wandb.log({"discrim_loss": discrim_loss})
                    wandb.log({"value_loss": value_loss})
                    print('loss:', discrim_loss, value_loss)
                    buffer = ReplayBuffer(buffer_size=config.buffer_size, batch_size=config.batch_size, device=device)

                if t != 0:
                    all = all + time.time()-st
                    print('avg_compute_time', all/t)
