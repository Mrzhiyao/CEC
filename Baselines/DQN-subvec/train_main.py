...
import glob
from utils import save, collect_random, collect_random_main_train_test
import random
...
def get_config():
    ...
    parser.add_argument("--run_name", type=str, default="DQN", help="Run name, default: DQN")
...
def train(config):
    ...
    agent = DQN(state_dim_actor, action_dim, actor_node, lr_actor, device)
    while 1:
        loss = agent.learn(steps, buffer.sample())
        print('loss', torch.mean(loss))
    with wandb.init(project="add-algorithm", name=config.run_name+'training-dqn', config=config):
        for xx in range(200):
            print('step:', xx)
