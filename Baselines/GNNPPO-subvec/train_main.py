...
import glob
from utils import save, collect_random, collect_random_main, collect_random_main_train_test
import random
from agent_gnn_ppo import GNNPPO
import env_main
...
from format_dataset import analyze_tasks, find_shared_references
...
def get_config():
    ...
    parser.add_argument("--run_name", type=str, default="GNNPPO", help="Run name, default: GNNPPO")
    ...
    parser.add_argument("--batch_size", type=int, default=80, help="Batch size, default: 256")
...
def train(config):
    ...
    agent_config = {
        ...
        'edge_dim': 3,
        'global_dim': 6,
        'action_dim': 4,  # 4种可能的优先级级别
        'hidden_dim': 512,
        'lr': 1e-4,
    }
    agent = GNNPPO(agent_config, state_dim_actor, action_dim, actor_node, lr_actor, device)

    with wandb.init(project="add-algorithm", name=config.run_name, config=config):
        ...
        for i in range(0, config.episodes):
            ...
            for t in range(500):            
                ...
                edge_features = analyze_tasks(task_content_list)
                edge_index = find_shared_references(task_content_list)

                if edge_index == []:
                    end = 0
                    # 遍历所有可能的起始任务索引
                    for start in range(len(edge_features[0])):
                        # 计算结束任务索引 (当前任务+1)
                        end = start + 1
                        # 当结束索引小于任务总数时，创建连接
                        if end < len(edge_features[0]):
                            edge_index.append([start, end])
                elif len(edge_index) < len(edge_features[0]) - 1:
                    # 计算所有可能的连续边
                    required_edges = [[i, i+1] for i in range(len(edge_features[0]) - 1)]
                    
                    # 合并现有边和缺失边
                    combined_edges = set(map(tuple, edge_index + required_edges))
                    
                    # 排序处理
                    edge_index = sorted(combined_edges, key=lambda x: x[0])
                    edge_index = [list(e) for e in edge_index]
                ...
                for m in range(len(state)):
                    ...
                    gnn_state.append(result_state_add)
                
                edge_index_t = [
                    [edge[0] for edge in edge_index],  # 所有源节点
                    [edge[1] for edge in edge_index]   # 所有目标节点
                ]

                actions1, action2, gnn_task_state = agent.get_action(gnn_state, edge_index_t, edge_features)
                add_gnn_task_state.append(gnn_task_state)
                for mmm in range(len(state)):
                    result_state_add = gnn_state[mmm]
                    action_cost.append(actions1[mmm])
                    actions2.append(action2)
                    state_new.append(state[mmm])   

                steps += 1
                state_mem.append(state_new)
                action_mem.append(actions1.tolist())

                if len(buffer)>100:
                    for ttt in range(10):
                        loss, output_policy_loss, output_value_loss, output_loss = agent.learn(t, buffer.sample())
                        wandb.log({"output_policy_loss": output_policy_loss.item()})
                        wandb.log({"output_value_loss": output_value_loss.item()})
                        wandb.log({"output_loss": output_loss.item()})
                st = time.time()
                ...
                for n in range(len(state_relation)):
                    # print('state_relation', state_relation)
                ...
                if (t+1) % 10 == 0 and t != 0:
                    ...
                    for task_file in tasks_list:
                        ...
                        buffer.add(state=add_gnn_task_state[add_number], action=action_mem[add_number], reward=rewards_mem-action_mem[add_number].count(3) * 30, next_state=add_gnn_task_state[add_number], done=0)
